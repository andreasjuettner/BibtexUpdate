...
## Modifications 12/2020 juttner@soton.ac.uk
## to adjust for new inspirehep API. I only fixeBTex - other methods not checked.d 
...
import sys, os, getopt, re, urllib, json, urllib.request
...
def getBiBTeX(ref,rType):
    ...
    # inspire changed their API in 2020 - the following should work
#    print 'https://inspirehep.net/api/literature?q=texkey+' + ref + '&format=bibtex'
    if rType=='texkey':
     #BiBTeX = urllib.urlopen('https://inspirehep.net/api/literature?q=texkey+' + ref + '&format=bibtex')
     print(rType,ref)
     url = 'https://inspirehep.net/api/arxiv/' + ref + '?format=texkey'
     parts = ref.split(':')
     url = 'https://inspirehep.net/api/literature?sort=mostrecent&size=25&page=1&q='+parts[0]+'%3A'+parts[1]+'&ui-citation-summary=true'
     url = 'https://inspirehep.net/api/literature/?sort=mostrecent&q='+parts[0]+'%3A'+parts[1]+'&size=25&page=1&format=bibtex'
    else:
     print('https://inspirehep.net/api/arxiv/' + ref + '?format=bibtex')
     url = 'https://inspirehep.net/api/arxiv/' + ref + '?format=bibtex'
    print(url)
    BiBTeX = urllib.request.urlopen(url)
#     BiBTeX = url.read()
     #BiBTeX = urllib.urlopen('https://inspirehep.net/api/arxiv/' + ref + '?format=bibtex')
